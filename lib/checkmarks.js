...
function checkRoot () {
    // adds a checkmark to the root person
    persons = document.querySelectorAll('input[type=checkbox]')
    for (var i=0;i<persons.length;i++) if (persons[i].title === mark) persons[i].checked = true
    }
...
function showFromList () {
    // displays on the left the people in a list
    
    const textarea = document.getElementById('listOfCheckedNames')

    // Split into lines, trim, and filter out blanks
    ...
    // Add to a Set
    const nameSet = new Set(items)

    // Confirm:
    console.log(nameSet)
    
    // display only those lines with matching id
    const nodes = document.querySelectorAll('.n, .spousen, .personRow')
    console.log(nodes)
    for (let i=0; i<nodes.length; i++) {
        if (nameSet.has(nodes[i].querySelector('.personsID').textContent)) nodes[i].style.display = 'block'
