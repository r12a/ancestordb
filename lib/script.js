...
function assignHelpTextX () {
    ...
    attachHelpPopup( document.getElementById('tagIndicator'),
    "Click to toggle whether this person is tagged. You can tag people for any reason." )
...
function summarise (node, thisPerson) {
    ...
    window.eventSummaryList = 'Age\tName\tEvent\tLocation\tOccupation\n'
    for (g = 1; g < entries.length; g++) {
        const age = entries[g].querySelector('.recordTitleAge') ? entries[g].querySelector('.recordTitleAge').textContent.trim() : '—'
        ...
        const title = entries[g].querySelector('.recordTitle') ? entries[g].querySelector('.recordTitle').textContent.trim() : ''
...
function getAllSiblings(person) {
    ...
    for (const s of ordered) {
        ...
        if (s === person) {
            siblings += `<li style="text-transform:uppercase; color:#648200; font-size:.9rem; font-weight:bold;"><img src="lib/i/copytiny.svg" style="height:1em; width:1em" onclick="copyThis('${ person }')"> ${ getName('', person, 'given', false, '') }</li>`
            continue
            }
...
function getAllSiblingsXXX(person) {
    // produce a list of siblings that will be used at the top right of the page
    
    // Safely check that the person exists in the database
    if (!db[person]) {
        console.warn(`getAllSiblings: person '${ person }' not found in db`)
        return '<ul></ul>'
        }

    // Determine which parent record to use (prefer father, fall back to mother)
    let parent = null

    if (db[person].father && db[db[person].father]) parent = db[db[person].father]
    else if (db[person].mother && db[db[person].mother]) parent = db[db[person].mother]

    // If no valid parent or no family groups, return empty list
    if (!parent || !parent.fg) {
        return '<ul></ul>'
    }

    // Use a Set to avoid duplicates if a child appears in multiple family groups
    const sibSet = new Set()

    // Collect siblings from all family groups
    for (let i = 0; i < parent.fg.length; i++) {
        const fam = parent.fg[i]

        // Children begin at index 2 (0 = father, 1 = mother)
        for (let j = 2; j < fam.length; j++) {
            const s = fam[j]
            if (s !== person) {
                sibSet.add(s)
    ...
    for (const s of sibSet) {
        // Known person in db → use getName()
        if (db[s]) {
            siblings += `<li><img src="lib/i/copytiny.svg" style="height:1em; width:1em" onclick="copyThis('${ s }')"> ${ getName('', s, 'given', true, '') }</li>`
        }
...
function getAllSiblingsXX (person, divider) {
...
function redisplay (researchNotes, id, summary ) { 
    ...
    /*out += `<span id="stopHelp" 
                title="Stop showing help popups." onclick="window.helpPopups=false" 
                style="margin-block-start:8rem;"
                ><img src="lib/i/stopHelp.png" alt="Stop the help popups." style="height:2rem; cursor:pointer;"></span>`*/
    
    out += `<span id="toggleHelp" 
                title="Toggle help popups." style="margin-block-start:8rem;"><img id="helpToggleIcon" src="lib/i/stopHelp.png" alt="Toggle help popups" style="height:2rem; cursor:pointer;"></span>`
	...
	out += '<div id="summary" class="record" style="background-color:transparent;"><div><p>' // the extra div keeps all together, and vertical
    ...
    if (typeof location_summary !== 'undefined') out += `<p id="location_summary"><a target="_blank" href="${ location_summary.url }">${ location_summary.list.replace(/,/g,' \u2022 ') }</a></p>`
    // with mousover...if (typeof location_summary !== 'undefined') out += `<p id="location_summary"><a target="_blank" href="${ location_summary.url }" onmouseover="helpDialog.textContent = 'Click to show places this person lived, on a map.'; helpDialog.style.display='block'" onmouseout="helpDialog.style.display = 'none'">${ location_summary.list.replace(/,/g,' \u2022 ') }</a></p>`
    
    // add the life summary narrative
    var intro = ''
    
    if (narrative && narrative !== '') { 
        intro += `<section id="life_summary" class="life_summary">`
        intro += narrative
        intro += `</section>`
        }

    else if (person.intro) intro += person.intro
        ...
        if (person.b) {
            if (person.b.includes('*')) lifeSummary += ` is likely to have been born around ${ person.b }`
            else lifeSummary += ' was born in '+person.b
            }
        if (person.bplace) lifeSummary += ' at '+person.bplace
        lifeSummary += '. '
        if (person.fg) {
            for (i=0;i<person.fg.length;i++) {
                ...
                if ((person.fg[i][0] === '' || person.fg[i][1] === '?') && person.fg[i].length > 2) {
                    ...
                    lifeSummary += person.pron+' married '+getName('', person.fg[i][1], 'gf', false)+' in '+person.fg[i][0].substr(0,4)
        ...
        if (person.d) {
            ...
            else lifeSummary += person.pron+' died in '+person.d
            if (person.dplace) lifeSummary += ' at '+person.dplace
            var ed = ey = bd = by = '?'
            if (person.b) {
    ...
    /*if (typeof places !== 'undefined' && places !== '') out += `<p id="summaryToggle" onclick="toggleLifeSummary();">\u2190 Show location breadcrumbs</p>`
    
    
    
    out += `<p id="narrative_button"><button onclick="summaryNode = document.querySelectorAll('.life_summary')[1].innerHTML = narrative">Narrative descriptions</button></p>`
    
    out += `<p id="esummary_button"><button onclick="summaryNode = document.querySelectorAll('.life_summary')[1]; summaryNode.innerHTML = makeEventTable(window.eventSummaryList)">Summary event list</button></p>`
    
    if (typeof places !== 'undefined' && places !== '') out += `<p id="breadcrumb_button"><button onclick="showBreadcrumbs()">Location breadcrumbs</button></p>`*/

out += `<div id="summaryToggle">`
out += `\u2190 Show`
// Narrative radio button
out += '<p id="narrative_button">'
out += `<label><input type="radio" name="summaryMode" checked onclick="document.querySelectorAll('.life_summary')[1].innerHTML = narrative">Narrative descriptions</label>`
out += '</p>'

// Event summary radio button
out += '<p id="esummary_button">'
out += `<label><input type="radio" name="summaryMode" onclick="document.querySelectorAll('.life_summary')[1].innerHTML = makeEventTable(window.eventSummaryList)">Event list summary</label>`
out += '</p>'

// Breadcrumb radio button (only if places exists)
if (typeof places !== 'undefined' && places !== '') {
	out += '<p id="breadcrumb_button">'
	out += `<label><input type="radio" name="summaryMode" onclick="showBreadcrumbs()">Location breadcrumbs</label>`
	out += '</p>'
	}
out += `</div>`
...
function showCensusEntriesX (person) {
...
function convertLineToRowXXXX (line) {
    // 0. Extract the age at the start (digits before the first comma)
    const ageMatch = line.match(/^(\d+),/);
    const age = ageMatch ? ageMatch[1] : '';
    const lineWithoutAge = ageMatch ? line.slice(ageMatch[0].length) : line;
    ...
    const quoted = quotedMatch[1]; // full quoted text without quotes
    ...
    // if the location is the same as for the previous row, use ditto mark
    let displayBeforeColon = beforeColon;
    if (lastBeforeColon !== null && beforeColon === lastBeforeColon) {
        displayBeforeColon = '\u00A0 \u3003';
    }
    ...
    // 5. Build the Google Maps link cell
    const mapLink = `<a href="https://www.google.com/maps?q=${lat},${lon}" target="_blank">GO</a>`;

    // 6. Build the <tr> with <td> cells (age added at the start)
    return `
<tr>
  <td style="color:#FFDEAF; font-size:1.4em;background:gray; padding-block:0; padding-inline:0.25em; text-align:center; border-radius:0.5em; vertical-align:middle;">${age}</td>
  <td>${year}</td>
  <td>${displayBeforeColon}</td>
  <td>${afterColon}</td>
  <td style="white-space:pre; padding-inline:0.2em; font-size: 80%; text-align:end;">${finalVal}</td>
  <td style="padding-inline:0;">${mapLink}</td>
</tr>`
}
...
function makeEventTableX (str) {
	...
	// Track previous age and year for blanking logic
	let prevAge = ''
	let prevYear = ''
	
	// Begin table
	let out = '<table class="listTable" style="font-size: 80%; margin-inline: 5rem;">'
