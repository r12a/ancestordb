...
function summariseX (node, thisPerson) {
	var out = ''
...
*/
    ...
    out += '<p>Copy to clipboard '
    // make a list of GPS links for use in the intro
    gps = document.querySelectorAll('.gpslink')
    gpsSet = new Set([])
    for (g=0;g<gps.length;g++) {
        if (gps[g].closest('.dateAndRecord').style.display !== 'none') coord = `${ gps[g].textContent.trim() }§${ gps[g].href }`
        //coord = `${ gps[g].textContent.trim() }§${ gps[g].href }`
        gpsSet.add(coord)
        }
    gpsArray = [...gpsSet]
    
    // create a button to copy individual links to an intro
    gpsLinks = '&lt;p&gt;\n'
    ...
    out += `<button 
            onclick="navigator.clipboard.writeText(\`${ gpsLinks }\`); 
           document.getElementById('copyNotice').style.display='block'; 
           setTimeout(()=>{document.getElementById('copyNotice').style.display='none'},500)">
            GPS links
            </button>`
    ...
    // create a button to create a .csv file for My Maps PROBABLY NOT NEEDED, GIVEN THE BUTTON THAT FOLLOWS
    /*gpsLinks = 'Name,Latitude,Longitude\n'
    for (g=0;g<gpsArray.length;g++) {
        temp = gpsArray[g].split('§')
        gpsLinks += `${ temp[0].replace(/,/g,'') },${ temp[1].replace(/https:\/\/maps.google.com\/maps\?q=/,'').replace(/%20/g,'') }\n`
        }
    ...
    out += `<button 
           ...
           document.getElementById('copyNotice').style.display='block'; 
           setTimeout(()=>{document.getElementById('copyNotice').style.display='none'},500)">
            Copy My Maps .csv text to clipboard
            </button>`
            */

    out += '&nbsp;&nbsp;'

    // create a button to create a .csv file for My Maps that lists one table row for each dateAndRecord
    ...
    for (g=1;g<entries.length;g++) {
        ...
        gpsLink = entries[g].querySelector('.gpslink')?entries[g].querySelector('.gpslink').href:''
        coords = gpsLink.split('?')
        if (typeof coords !== 'undefined' && [...coords].length > 1) {
            coords = coords[1]
            coords = coords.replace(/q=/,'')
            }
        else coords = '&#x2014;HELLO'
        title = entries[g].querySelector('.recordTitle')?entries[g].querySelector('.recordTitle').textContent.trim():''
        if (entries[g].querySelector('.occ') && entries[g].querySelector('.occ').textContent !== '') occupation = ' \u2022 '+entries[g].querySelector('.occ').textContent.trim()
        else occupation = ''
        
        gpsLinks += `"${ year } ${ locn }: ${ title }${ occupation }",${ coords }\n`
        }
    ...
    out += `<button id="copyCsvBtn">Detailed .csv data</button>`
    ...
    document.getElementById("copyCsvBtn").addEventListener("click", () => {
        navigator.clipboard.writeText(gpsLinks)
...
function summariseXX(node, thisPerson) {
    ...
    // -----------------------------
    // BUILD THE SUMMARY HTML (unchanged)
    // -----------------------------
    out += '<h4>BMD</h4><ul>'
    if (db[thisPerson].b) {
        out += `<li style="font-size: 1.4em;">${db[thisPerson].b}`;
    ...
    out += ` &nbsp; <button id="copyGpsBtn">GPS links</button> &nbsp; `

    // Detailed CSV
    entries = document.querySelectorAll('.dateAndRecord')
    let csvText = 'Name,Latitude,Longitude,Precision\n'
    for (g = 1; g < entries.length; g++) {
        year = entries[g].querySelector('.theYear') ? entries[g].querySelector('.theYear').textContent.trim() : ''
        locn = entries[g].querySelector('.place') ? entries[g].querySelector('.place').textContent.trim() : ''
        ...
        console.log(coords)
        if (coords.length > 1) {
            coords = coords[1].trim().replace(/q=/, '')
            const [coords, cflag] = coords.split("\u00A7")
            }
        else coords = '\u2014'*/
            
/*
<a target="_blank" class="gpslink" href="https://maps.google.com/maps?q=53.384384097153244,-2.7274932381566686"><img src="lib/i/map.png" alt="" title="St Lukes, Farnworth, Widnes, Lancs:exact"> St Lukes, Farnworth, Widnes, Lancs</a>
*/
        ...
        coords = '\u2014'
        precision = 'undefined'
        const gpsLink = entries[g].querySelector('a.gpslink')

        if (gpsLink) {
        // Extract coords from href
            const href = gpsLink.getAttribute('href')
            ...
            const anchortitle = gpsLink.querySelector('img').getAttribute('title')
            ...
            console.log('COORDS', coords, 'precision', precision)
            }
        ...
        //csvText += `"${ year } ${ locn }: ${ title }${ occupation }",${ coords },\u{1F4CC} ${ cflag }\n`
        csvText += `"${ year } ${ locn }: ${ title }${ occupation }",${ coords },\u{1F4CC} ${ precision }\n`
        }
...
function summarise (node, thisPerson) {
    ...
    for (g = 1; g < entries.length; g++) {
       ...
       // create a line for each gps link, including the above information and the GPS-specific info
        ...
        else csvText += `"${ year } ${ locn }: ${ title }${ occupation }",\u2014,\u{1F4CC} undefined\n`
        }
...
function addFIcon (text, type) {
	...
	if (text) {
		...
		if (type === 'gps') {
            ...
            const label = text.slice(0, urlStart).trim()
            const urlAndFlag = text.slice(urlStart)
            const [url, flag] = urlAndFlag.split("\u00A7")
			out = `<p class="floatedIconText"><a target="_blank" class="gpslink" href="${ url }"><img src="lib/i/map.png" alt="" title="${ label }\u00A7${ flag }"> ${ label }</a></p>`
			//out = `<p class="floatedIconText"><a target="_blank" class="gpslink" href="http${ subparts[0] }"><img src="lib/i/map.png" alt="" title="${ parts[0] }"> ${ parts[0] }</a></p>`
            //console.log("GOING OUT",out)
			return out.trim()
			}
		if (type === 'record') {
...
function getTimestampX (date, year) {
...
function getAllSiblingsX (person, divider) {
    // returns markup containing siblings of person, with links
    // divider is markup or string to be added between each name
    // console.log('getSiblings(',person,')')
...
function redisplay (researchNotes, id, summary ) { 
	...
	out += `<a title="Go to the Index." target="_blank" href="index.html?project=${ project }"
                onmouseover="helpDialog.textContent = 'Show the index.'; helpDialog.style.display='block'"
                onmouseout="helpDialog.style.display = 'none'"
                >&#x1F4C7;</a>`

    out += `<a id="gotoTree" target="_blank" title="Show Relationships" href="tree.html?project=${ project }&person=${ id }"
                onmouseover="helpDialog.textContent = 'Show two generations of ancestors, and the spouses and children for this person.'; helpDialog.style.display='block'"
                onmouseout="helpDialog.style.display = 'none'"
                >&#x23F3;</a>`
     
	out += `<a title="Show Ancestors" target="_blank" style="opacity:80%;" href="precedents.html?project=${ project }&person=${ id }"
                onmouseover="helpDialog.textContent = 'Show all known ancestors of this person.'; helpDialog.style.display='block'"
                onmouseout="helpDialog.style.display = 'none'"
                >&#x23EB;</a>`
   
	out += `<a title="Show Descendants." target="_blank" style="opacity:80%;" href="descendants.html?project=${ project }&person=${ id }"
                onmouseover="helpDialog.textContent = 'Show all known descendants of this person.'; helpDialog.style.display='block'"
                onmouseout="helpDialog.style.display = 'none'"
                >&#x23EC;</a>`
    
	if (db[id].a) out += `<a title="Show in Ancestry.com family tree." style="opacity:80%;" href="javascript:void(0)" onclick="ancestry = '${ db[id].a }'.split(','); openWindow(\'https://www.ancestry.co.uk/family-tree/tree/'+ancestry[0]+'/family?cfpid='+ancestry[1])"><img 
                src="lib/i/chart.png" 
                style="height:1.4em; width:1.4em;"
                onmouseover="helpDialog.textContent = 'Find this person in an Ancestry.com family tree.'; helpDialog.style.display='block'"
                onmouseout="helpDialog.style.display = 'none'"
                ></a>`
    
	out += `<a title="Show Family Tree" target="_blank" href="ancestors.html?project=${ project }&person=${ id }"
                onmouseover="helpDialog.textContent = 'Show ancestors of this person laid out like a family tree.'; helpDialog.style.display='block'"
                onmouseout="helpDialog.style.display = 'none'"
                >&#x1F333;</a>`
    
	out += `<a title="Search for entries" target="searchWindow" href="search.html?person=${ id }&project=${ project }"
                onmouseover="helpDialog.textContent = 'Search for places, occupations, or events.'; helpDialog.style.display='block'"
                onmouseout="helpDialog.style.display = 'none'"
                >&#x1F50D;</a>`
	...
	out += `<a title="Copy id to clipboard" style="opacity:80%;" href="javascript:void(0)"><img 
                ...
                onmouseover="helpDialog.textContent = 'Copy the id of this person to the clipboard.'; helpDialog.style.display='block'"
                onmouseout="helpDialog.style.display = 'none'"
                ></a> `
	...
	out += `<img 
                ...
                onmouseover="helpDialog.textContent = 'Open a Person Record entry form.'; helpDialog.style.display='block'"
                onmouseout="helpDialog.style.display = 'none'"
                >`
    ...
    out += `<span id="tagIndicator" 
                ...
                style="font-size: 100%; line-height:1; color:brown; cursor:pointer;"
                onmouseover="helpDialog.textContent = 'Click to toggle whether this person is tagged. You can tag people for any reason.'; helpDialog.style.display='block'"
                onmouseout="helpDialog.style.display = 'none'"
                >`
	...
	out += '<div id="summary" class="record" style="background-color:transparent;"><div><p>'
    ...
    if (person.intro) {
        intro += `<div style="font-size:1.1rem;">${ person.intro }</p>`
        if (person.cstatus) intro += '<p style="color:#aaa; text-align:center;margin-top:1em;">&#x2014; '+person.cstatus+' &#x2014;</div>'
        }
    else {
        intro += `<div style="font-size:1.1rem;"><p id="location_summary">${ person.bplace } \u2022 ${ person.dplace }</p>`
        intro += `<section class="life_summary">`
        intro += getName('', id, 'gfm', false)
        if (person.k) intro += ', known as '+person.k+', '
        if (person.b) {
            if (person.b.includes('*')) intro += ` is likely to have been born around ${ person.b }`
            else intro += ' was born in '+person.b
            }
        ...
        if (person.d) {
            if (person.d.includes('*')) intro += person.pron+' is likely to have died around '+person.d
            else intro += person.pron+' died in '+person.d
    ...
    out += summary.innerHTML+'</div></div><div class="keypoints" style="margin-block-start:4rem;">'
    ...
    //if (birthNote !== '' && deathNote !== '') out += `<div id="headsup_age">Aged: ${ getAge("",person.ddate,person.d,person.bdate,person.b,"") }</div>`
    out += `</div>`
...
// THESE MAY NEED TO BE CHANGED TO birthTS and deathTS respectively, if actually used
    ...
    for (var e=0;e < events.length; e++) {
        ...
        if (event.type === 'census') {
            ...
            // create a summary
            /*record += `<p style="font-size:80%; color:gray;">`
            if (event.head) {
                summaryname = event.head.split(';')[0]
                if (typeof db[summaryname] !== 'undefined') summaryname = db[summaryname].g + ' ' + db[summaryname].f
                record += summaryname
                }
            if (event.wife) {
                record += ` & `
                summaryname = event.wife.split(';')[0]
                if (typeof db[summaryname] !== 'undefined') summaryname = db[summaryname].g
                record += summaryname
                }
            if (event.children) {
                record += '<br>'
                for (i=0;i<event.children.length;i++) {
                    if (i>0) record += ' \u2022 '
                    summaryname = event.children[i].split(';')[0]
                    if (typeof db[summaryname] !== 'undefined') summaryname = db[summaryname].g
                    record += summaryname
                    }
                }
            if (event.others) {
                ...
                for (i=0;i<event.others.length;i++) {
                    if (i>0) record += ' \u2022 '
                    summaryname = event.others[i].split(';')[0]
                    if (typeof db[summaryname] !== 'undefined') summaryname = db[summaryname].g + ' ' + db[summaryname].f
                    record += summaryname
                    }
                }
            if (event.serv) {
                ...
                for (i=0;i<event.serv.length;i++) {
                    if (i>0) record += ' \u2022 '
                    summaryname = event.serv[i].split(';')[0]
                    if (typeof db[summaryname] !== 'undefined') summaryname = db[summaryname].g + ' ' + db[summaryname].f
                    record += summaryname
                    }
                }
            if (event.visitors) {
                ...
                for (i=0;i<event.visitors.length;i++) {
                    if (i>0) record += ' \u2022 '
                    summaryname = event.visitors[i].split(';')[0]
                    if (typeof db[summaryname] !== 'undefined') summaryname = db[summaryname].g + ' ' + db[summaryname].f
                    record += summaryname
                    }
                }
...
*/
        ...
        if (event.type === 'born' && event.relation === 'self') {
            // console.log('redisplay: Self birth ',person, person.events[event.timestamp])
...
// info needed coming into this section: event.title, details, event.type/occ/place
        ...
        // draw age + year 
		//out += '<div class="dateAndRecord">'
        if (event.type === 'marriage') out += `<div class="dateAndRecord marries">`
		else out += `<div class="dateAndRecord ${ event.type }">`
...
function prepCopy () {
    ...
    /*titles = document.querySelectorAll('.recordTitle')
    for (i=0;i<titles.length;i++) {
        titles[i].innerHTML = '<span style="font-size: 1rem;">Record title:</span> '+titles[i].textContent
        }*/
