...
function redisplay (researchNotes, id, summary ) { 
    ...
    if (typeof location_summary !== 'undefined') out += `<p id="location_summary"><a target="_blank" href="${ location_summary.url }" onmouseover="helpDialog.textContent = 'Click to show places this person lived, on a map.'; helpDialog.style.display='block'" onmouseout="helpDialog.style.display = 'none'">${ location_summary.list.replace(/,/g,' \u2022 ') }</a></p>`
    ...
    if (typeof places !== 'undefined' && places !== '') out += `<p id="summaryToggle" onclick="toggleLifeSummary()">\u2190 Show location list</p>`
...
function toggleLifeSummary () {
    ...
    if (summaryToggle.textContent.trim() === '\u2190 Show location list') {
        placeLines = places.split('\n')
        out = '<table id="lifeSummaryTable" style="font-size:80%; margin-inline:5rem;">'
        out += '<tr><th>Age</th><th>Year</th><th>Event</th><th colspan="3">Location</th></tr>'
        ...
        document.getElementById('summaryToggle').textContent = '\u2190 Show location list'
        }
    }
...
function convertLineToRowX(line) {
    // 0. Extract the age at the start (digits before the first quote)
    const ageMatch = line.match(/^(\d+)\s+/);
    const age = ageMatch ? ageMatch[1] : '';
    const lineWithoutAge = ageMatch ? line.slice(ageMatch[0].length) : line;
