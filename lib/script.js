...
function redisplay (researchNotes, id, summary ) { 
    ...
    if (narrative && narrative !== '') narrativeAvailable = true
    ...
    if (narrativeAvailable) out += narrative

    else if (person.intro) {
        narrative = person.intro
        out += person.intro
        }
        
    else { // make a narrative
        let lifeSummary = ''
    ...
    out +=  `<div><img src="lib/i/map_pin.png" alt="Click to see on a map" style='height:1rem; margin-block-start:.5rem;'></div><p id="location_summary">`
    if (typeof location_summary !== 'undefined') out += `<a target="_blank" href="${ location_summary.url }">${ location_summary.list.replace(/,/g,'<br>').replace(/\u2022/g,'<br>') }</a>`
