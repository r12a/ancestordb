...
function summarise (node, thisPerson) {
    ...
    for (g = 1; g < entries.length; g++) {
        let age = entries[g].querySelector('.recordTitleAge') ? entries[g].querySelector('.recordTitleAge').textContent.trim() : '—'
        let year = entries[g].querySelector('.theYear') ? entries[g].querySelector('.theYear').textContent.trim() : ''
        let locn = entries[g].querySelector('.place') ? entries[g].querySelector('.place').textContent.trim() : ''
        let title = entries[g].querySelector('.recordTitle') ? entries[g].querySelector('.recordTitle').textContent.trim() : ''
        let occupation = entries[g].querySelector('.occ') && entries[g].querySelector('.occ').textContent !== '' ?
            ' \u2022 ' + entries[g].querySelector('.occ').textContent.trim() : ''
       ...
       // create a line for each gps link, including the above information and the GPS-specific info
        ...
        const gpsLinkData = entries[g].querySelectorAll('.place')
        if (gpsLinkData.length > 0) {
            for (l=0;l<gpsLinkData.length;l++) {
                // Extract coords from href
                let breadcrumb = gpsLinkData[l].dataset.bc
                if (typeof breadcrumb === 'undefined' || breadcrumb === '') continue
                else breadcrumb = breadcrumb.replace(/\u00A7/,',\u{1F4CC}')
                const locn = gpsLinkData[l].textContent

                csvText += `${ age },"${ year } ${ locn }: ${ title }${ occupation }",${ breadcrumb }\n`
                }
            }
        //else csvText += `X${ age },"${ year } ${ locn }: ${ title }${ occupation }",\u2014,\u{1F4CC} undefined\n`
        }
...
*/            
            ...
            // create a summary
            let censusSummary = `<div class="censusSummary">`
            
            censusSummary += `<p><img src="lib/i/date.png"" alt="Date"> ${ event.date } ${ event.year }</p>`
            
            censusSummary += `<p><img src="lib/i/place.png"" alt="Place"> ${ expandChapmanCodes( event.place.split('http')[0].trim() ) }</p>`
            
            if (event.head && event.wife) censusSummary += `<p><img src="lib/i/parent.png" alt="Head & wife"> `
            else if (event.head) censusSummary += `<p><img src="lib/i/parent.png" alt="Head"> `
            if (event.head) {
                summaryname = event.head.split(';')[1].trim()
                summaryage = event.head.split(';')[2].trim()
                if (typeof db[event.head.split(';')[0]] !== 'undefined') headSurname = new RegExp(db[event.head.split(';')[0]].f)
                else headSurname = ''
                censusSummary += `${ summaryname }<sup class="ageTag">${ summaryage }</sup>` 
                }
            if (event.wife) {
                censusSummary += ` &nbsp;&&nbsp; `
                summaryname = event.wife.split(';')[1].replace(headSurname,'').trim()
                summaryage = event.wife.split(';')[2].trim()
                censusSummary += `${ summaryname }<sup class="ageTag">${ summaryage }</sup>`
                }
            if (event.head || event.wife) censusSummary += `</p>`

            if (event.children) {
                censusSummary += `<p><img src="lib/i/children.png"" alt="Children"> `
                for (i=0;i<event.children.length;i++) {
                    //if (i>0) censusSummary += ' \u2022 '
                    if (i>0) censusSummary += ' \u00A0 '
                    summaryname = event.children[i].split(';')[1].replace(headSurname,'').trim()
                    summaryage = event.children[i].split(';')[2].trim()
                    censusSummary += `${ summaryname }<sup class="ageTag">${ summaryage }</sup>`
                    }
                censusSummary += `</p>`
                }

            //if (event.others || event.serv || event.visitors) censusSummary += `<p style="margin-block:-.5em"><img src="lib/i/others.png" alt="Others">`
            if (event.others) {
                censusSummary += `<p><img src="lib/i/others.png" alt="Others">  `
                for (i=0;i<event.others.length;i++) {
                    //if (i>0) censusSummary += ' \u2022 '
                    if (i>0) censusSummary += ' \u00A0 '
                    summaryname = event.others[i].split(';')[1].trim()
                    summaryage = event.others[i].split(';')[2].trim()
                    censusSummary += `${ summaryname }<sup class="ageTag">${ summaryage }</sup>`
                    }
                censusSummary += `</p>`
                }
            ...
            if (event.serv) {
                censusSummary += `<p><img src="lib/i/servants.png" alt="Servants">  `
                for (i=0;i<event.serv.length;i++) {
                    //if (i>0) censusSummary += ' \u2022 '
                    if (i>0) censusSummary += ' \u00A0 '
                    summaryname = event.serv[i].split(';')[1].trim()
                    summaryage = event.serv[i].split(';')[2].trim()
                    censusSummary += `${ summaryname }<sup class="ageTag">${ summaryage }</sup>`
                    }
                censusSummary += `</p>`
                }
            ...
            if (event.visitors) {
                censusSummary += `<p><img src="lib/i/visitors.png" alt="Others"> `
                for (i=0;i<event.visitors.length;i++) {
                    //if (i>0) censusSummary += ' \u2022 '
                    if (i>0) censusSummary += ' \u00A0 '
                    summaryname = event.visitors[i].split(';')[1].trim()
                    summaryage = event.visitors[i].split(';')[2].trim()
                    censusSummary += `${ summaryname }<sup class="ageTag">${ summaryage }</sup>`
                    }
                censusSummary += `</p>`
                }
            //if (event.others || event.serv || event.visitors) censusSummary += `</p>`
            censusSummary += `</div>`
            
            record += censusSummary
        ...
        if (event.type === 'dies' && event.relation === 'self') {
            ...
            if (event.namedInProbate) {
                ...
                record += `<p class="namedInProbate">Named in probate: ${ probatenames }.</p>`
                }
            if (event.notes) record += getNotes(event.notes, event.timestamp)
...
console.log('@@@',event.year, event.type.toUpperCase(), event)


        //KEYPOINTS (right-most column)
        ...
        if (event.relation === 'self' || 
            ...
            function renderPlace(raw, icon, extraAttrs = '') {
                if (!raw) return ''

                const [place] = raw.split('http')
                const placename = place.trim()
                if (!placename) return ''

                const q = raw.split('q=')[1]
                const coords = q ? ` data-bc="${q.trim()}"` : ''

                return `<div class="place"${coords}${extraAttrs}><img src="${icon}"> ${placename}</div>`
                }


            if (event.type === 'dies') {
                if (event.of) {
                    const athome = event.athome ? ` data-athome="${event.athome}"` : ''
                    out += renderPlace(event.of, 'lib/i/home.png', athome)
                    }
                if (event.place && event.place !== event.of) {
                    out += renderPlace(event.place, 'lib/i/map_pin_brown.png')
                    }
                if (event.burplace) {
                    out += renderPlace(event.burplace, 'lib/i/map_pin_brown.png')
                    }
                }
                ...
                if (event.place) {
                    const athome = event.athome ? ` data-athome="${event.athome}"` : ''
                    out += renderPlace(event.place, 'lib/i/home.png', athome)
                    }
                if (event.bapplace && event.relation === 'self') {
                    out += renderPlace(event.bapplace, 'lib/i/map_pin_brown.png')
                    }
                }

            else if (event.type === 'marriage') {
                if (person.male && event.gparish) {
                    out += renderPlace(event.gparish, 'lib/i/home.png')
                    }
                else if (event.bparish) {
                    out += renderPlace(event.bparish, 'lib/i/home.png')
                    }
                if (event.place) {
                    out += renderPlace(event.place, 'lib/i/map_pin_brown.png')
                    }
            ...
            else if (event.type === 'census') {
                if (event.place) out += renderPlace(event.place, 'lib/i/home.png')
                }
            ...
            /*if (event.type === 'dies') {
                if (event.of) {
                    placename = event.of.split('http')[0].trim() ? event.of.split('http')[0].trim() : ''
                    if (placename) coords = event.of.split('q=')[1] ? 'data-bc="'+event.of.split('q=')[1].trim()+'"' : ''
                    athome = event.athome ?  'data-athome="'+event.athome+'"' : ''
                    out += `<div class="place" ${ athome }${ coords }><img src="lib/i/home.png"> ${ placename }</div>`
                    }
                if (event.place  && event.place !== event.of) {
                    placename = event.place.split('http')[0].trim() ? event.place.split('http')[0].trim() : ''
                    if (placename) coords = event.place.split('q=')[1] ? ' data-bc="'+event.place.split('q=')[1].trim()+'"' : ''
                    out += `<div class="place"${ coords }><img src="lib/i/map_pin_brown.png"> ${ placename }</div>`
                    }
                if (event.burplace) {
                    placename = event.burplace.split('http')[0].trim() ? event.burplace.split('http')[0].trim() : ''
                    if (placename) coords = event.burplace.split('q=')[1] ? ' data-bc="'+event.burplace.split('q=')[1].trim()+'"' : ''
                    out += `<div class="place"${ coords }><img src="lib/i/map_pin_brown.png"> ${ placename }</div>`
                    }
                }
            else if (event.type === 'born') {
                if (event.place) {
                    ...
                    out += `><img src="lib/i/home.png" style="height:1em; vertical-align:text-top;"> ${ event.place.split('http')[0].trim() }</div>`
                    }
                if (event.bapplace) out += '<div class="place"><img src="lib/i/map_pin_brown.png" style="height:1em; vertical-align:text-top;"> '+event.bapplace.split('http')[0].trim()+'</div>'
                }
            else if (event.type === 'marriage') {
                if (person.male && event.gparish) out += '<div class="place"><img src="lib/i/home.png" style="height:1em; vertical-align:text-top;"> '+event.gparish.split('http')[0].trim()+'</div>'
                else if (event.bparish)  out += '<div class="place"><img src="lib/i/home.png" style="height:1em; vertical-align:text-top;"> '+event.bparish.split('http')[0].trim()+'</div>'
                if (event.place) {
                    out += `<div class="place"`
                    if (event.athome) out += ` data-athome="${ event.athome }"`
                    out += `><img src="lib/i/map_pin_brown.png" style="height:1em; vertical-align:text-top;"> ${ event.place.split('http')[0].trim() }</div>`
                    }              
                }
            else if (event.place) {
                out += `<div class="place"`
        ...
        let floatedIcons = ''  // build the html in this string
        if (event.images) {
        ...
        // only show the floating links if relevant to person page
        if (event.relation === 'self' || 
            event.type === 'born' ||
            event.type === 'census' ||
            event.type === 'note') {
...
/*
        if (event.of) {
            out += `<div class="place"`
            if (event.athome) out += ` data-athome="${ event.athome }"`
            out += `><img src="lib/i/home.png" style="height:1em;"> ${ event.of.split('http')[0].trim() }</div>`
            }
        else if (event.place) {
            out += `<div class="place"`
            if (event.athome) out += ` data-athome="${ event.athome }"`
        ...
        if (person.male && event.gparish) out += '<div class="place"><img src="lib/i/map_pin_brown.png" style="height:1em; vertical-align:middle;"> '+event.gparish.split('http')[0].trim()+'</div>'
        else if (event.bparish)  out += '<div class="place"><img src="lib/i/map_pin_brown.png" style="height:1em; vertical-align:text-top;"> '+event.bparish.split('http')[0].trim()+'</div>'
...
function showBreadcrumbs () {
    ...
    //out = `<p style="margin-block-end: 2rem; font-size: 90%;">Breadcrumbs are events that indicate where this person was living. (Unless contradictory evidence is available, it is assumed that a child lived in the same place that brothers & sisters were born.) For some people, a link in the box under the <img src="lib/i/map_pin.png" alt="Click to see on a map" style="height:.8rem; vertical-align: middle;"> icon to the right opens a map that plots all locations.</p>`
    out = `<p style="margin-block-end: 2rem; font-size: 80%;">Breadcrumbs are events that indicate where this person was living. Unless contradictory evidence is available, it is assumed that a child lived in the same place that brothers & sisters were born.`
    if (document.getElementById('location_summary').querySelector('a') !== null) out += ` The link in the box under the <img src="lib/i/map_pin.png" alt="Click to see on a map" style="height:.8rem; vertical-align: middle;"> icon to the right opens a map that plots all locations.`
    out += `</p>`
