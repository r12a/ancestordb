...
function tabs2colons (text) {
    ...
    for (i=0;i<lines.length;i++) {
        ...
        if (lines[i].trim() === 'Spouse:' && lines[i+1]) lines[i] += lines[i+1]
        if (lines[i].trim() === 'Father:' && lines[i+1]) lines[i] += lines[i+1]
        if (lines[i].trim() === 'Mother:' && lines[i+1]) lines[i] += lines[i+1]
        }
